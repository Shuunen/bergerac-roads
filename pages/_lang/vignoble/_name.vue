      ...
      <div class="encart" v-loading="loading">
        ...
        <template v-else-if="vineyardExists">
          ...
          <div>
            ...
            <ul>
              <li v-for="(element, index) in $t(`vineyards.${name}.numbers`)" :key="index">{{ element }}</li>
            </ul>
            <h2 class="subtitle">{{ $t('vineyards.titles.varieties') }}</h2>
            <ul>
              <li v-for="(element, index) in $t(`vineyards.${name}.varieties`)" :key="index">{{ element }}</li>
            </ul>
            <h2 class="subtitle">{{ $t('vineyards.titles.typicity') }}</h2>
            <ul>
              <li v-for="(element, index) in $t(`vineyards.${name}.typicity`)" :key="index">{{ element }}</li>
            </ul>
          </div>
        </template>
        <template v-else>
          ...
          <div>
            ...
            <ul>
              <li v-for="(vineyard, index) in vineyards" :key="index">
                <nuxt-link :to="$i18n.path(`vignoble/${vineyard.name}`)">{{ $t(`vineyards.${vineyard.name}.title`) }}</nuxt-link>
              </li>
            </ul>
          </div>
        </template>
